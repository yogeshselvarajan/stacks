...
describe("ApprovalCaseDetail", () => {
  it("populated state: shows the case summary and candidate list", () => {
    render(<ApprovalCaseDetail case={RED_CASE} onApprove={vi.fn()} onDecline={vi.fn()} onEdit={vi.fn()} actionStatus="idle" />);
  ...
  it("calls onApprove with no edited value on a plain Approve click", () => {
    ...
    render(<ApprovalCaseDetail case={YELLOW_CASE} onApprove={onApprove} onDecline={vi.fn()} onEdit={vi.fn()} actionStatus="idle" />);
  ...
  it("Decline requires a reason and calls onDecline with it", () => {
    ...
    render(<ApprovalCaseDetail case={YELLOW_CASE} onApprove={vi.fn()} onDecline={onDecline} onEdit={vi.fn()} actionStatus="idle" />);
  ...
  it("the Confirm decline button stays disabled until a reason is entered", () => {
    render(<ApprovalCaseDetail case={YELLOW_CASE} onApprove={vi.fn()} onDecline={vi.fn()} onEdit={vi.fn()} actionStatus="idle" />);
    fireEvent.click(screen.getByRole("button", { name: "Decline" }));
  ...
  it("calls onEdit with the chosen candidate id on Confirm edit", () => {
    const onEdit = vi.fn();
    render(<ApprovalCaseDetail case={RED_CASE} onApprove={vi.fn()} onDecline={vi.fn()} onEdit={onEdit} actionStatus="idle" />);
    fireEvent.click(screen.getByRole("button", { name: "Edit" }));
  ...
  it("loading (submitting) state disables every action button", () => {
    render(<ApprovalCaseDetail case={YELLOW_CASE} onApprove={vi.fn()} onDecline={vi.fn()} onEdit={vi.fn()} actionStatus="submitting" />);
    expect(screen.getByRole("button", { name: /approving/i })).toBeDisabled();
    expect(screen.getByRole("button", { name: "Decline" })).toBeDisabled();
  });

  it("error state: shows a specific error message, not a generic one", () => {
    render(<ApprovalCaseDetail case={YELLOW_CASE} onApprove={vi.fn()} onDecline={vi.fn()} onEdit={vi.fn()} actionStatus="error" />);
